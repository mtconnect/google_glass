...
class MainHandler(webapp2.RequestHandler):
  ...
  def _render_template(self, message=None):
    ...
    for stream in streams:
        ...
        streams = stream.getElementsByTagName("DeviceStream")
        for element in elements:
            # Grab data from the station element
            spindle = element.getElementsByTagName("SpindleSpeed")[0].firstChild.data
            estop = element.getElementsByTagName('EmergencyStop')[0].firstChild.data
            power = element.getElementsByTagName('PowerState')[0].firstChild.data
            
            # Append the data onto the string
            
            string = """<figure><img src="/uploads/221fummx1nch.jpg"></figure><section><table class="text-small align-justify"><tbody><tr><td>DMG</td><td>""" + str(spindle) + "</td></tr><tr><td>Utilization</td><td>" + str(estop) + "</td></tr><tr><td>Spindle Override</td><td>" + str(power) + "</td></tr></tbody></table></section>"
            
    template_values['xml'] = string
